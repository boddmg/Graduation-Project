...
def rbm_train(rbm, src_data = None, learning_rate=0.1, training_epochs=5, batch_size=100):
    ...
    """
    src_data = src_data.reshape(src_data.shape[0],src_data.shape[2])
    train_set_x = theano.shared(value=src_data, name='train_set_x', borrow=True)
    ...
    train_rbm = theano.function(
        ...
        updates=updates,
        givens={
            rbm.input: train_set_x[index * batch_size: (index + 1) * batch_size]
        },
        name='train_rbm'
    )
    ...
    for epoch in xrange(training_epochs):
        ...
        print 'Time:%d, training epoch %d, cost is ' % (epoch_time, epoch), numpy.mean(mean_cost)
